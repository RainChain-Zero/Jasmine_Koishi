...
const samplers = ['k_euler_ancestral', 'k_euler', 'k_lms', 'plms', 'ddim']
...
export const Config: Schema<Config> = Schema.object({
  ...
  baseTags: Schema.string().description('默认的附加标签。').default('best quality'),
...
export function apply(ctx: Context, config: Config) {
  ...
  const cmd = ctx.guild('921454429', '436159372')
    .command('novelai <prompts:text>')
    .alias('nai')
